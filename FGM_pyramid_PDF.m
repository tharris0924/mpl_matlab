...
function [f F K c r]=FGM_pyramid_PDF(b)
% b=[.3 .7];
c=FGM_pyramid_alpha(b);
t=length(0:.05:1);
X=[];
...
for i=1:na1
for j=1:nb1
f1(i,j)=c*(2*x1(i)+2*x2(j)-4*x1(i)*x2(j))/b(1)/(1-b(1))/4;
end
end
...
for j=1:nb2
a=4*b(2)*(1-b(2));
f3(i,j)=c*(2*(1-x1(i))+2*(1-x2(j))-4*(1-x1(i))*(1-x2(j)))/a;
end
end
...
%c0=(1-c(1)*b(1)^2/2-c(3)*b(1)*(1-b(2))/2-c(4)*(1-b(2))^2/2-c(2)*b(2)*(1-b(1))/2)/(b(2)-b(1))^2;
%for i=1:n1
%for j=1:n2
%f0(i,j)=c0;
%end
