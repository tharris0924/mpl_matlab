...
function [X, f, F, K, c0, r]=mpl_pdf(b,c)
 % b=[.3 .7];c=[.1 .2 .3 .4];
t=length(0:.05:1);
X=[];
...
for i=1:n1
for j=1:n2
f2(i,j)=c(3)*(2*x1(i)+2*(1-x2(j))-4*x1(i)*(1-x2(j)))/b(1)/(1-b(2));
end
end
...
for i=1:nb1
for j=1:nb2
f3(i,j)=c(4)*(2*(1-x1(i))+2*(1-x2(j))-4*(1-x1(i))*(1-x2(j)))/b(2)/(1-b(1));
end
end
...
for i=1:n1
for j=1:n2
f4(i,j)=c(2)*(2*(1-x1(i))+2*x2(j)-4*(1-x1(i))*x2(j))/(1-b(2))^2;
end
end
x1=b(1)+.05:.05:b(2)-.05;
x2=b(1)+.05:.05:b(2)-.05;
X=[X;x1' x2'];
n1=length(x1);n2=length(x2);
c0=(1-c(1)*(1+b(1)-b(1)^2)-c(2)*(1+b(1)-b(1)*(1-b(2)))-c(3)*((1-b(1))+1-b(2)*(1-b(1))))/(b(2)-b(1))^2;
%0=(1-c(1)*b(1)^2/2-c(3)*b(1)*(1-b(2))/2-c(4)*(1-b(2))^2/2-c(2)*b(2)*(1-b(1))/2)/(b(2)-b(1))^2;
for i=1:n1
for j=1:n2
f0(i,j)=c0;
end
...
ylabel('$x_2$', 'FontName', 'Times New Roman', ...
       'Color','k', 'Interpreter', 'LaTeX')
zlabel('Probability Density, $f(x_1,x_2)$','FontName', 'Times New Roman', ...
       'Color','k', 'Interpreter', 'LaTeX')
